...
> **状态**：v0.3.0——阶段 1（P0 修复）与阶段 2（标准布局重构）已完成：单入口 `workflow/Snakefile`（`pipeline=upstream|deg|as|lncrna`）、conda 环境入库、路径全参数化、全流程 MultiQC、`software_versions.yaml` 版本记录。遗留 P1/P2 见 [docs/审查报告.md](docs/审查报告.md)。
...
| [docs/优化路线图.md](docs/优化路线图.md) | 三阶段改造计划与验收标准（阶段 0/1/2 已完成，阶段 3 测试与 CI 待启动） |
| [CHANGELOG.md](CHANGELOG.md) | 版本变更记录 |

## 已知问题（v0.3.0 修复情况）

阶段 1 已修复全部 8 个 P0；阶段 2 完成结构标准化、统一 trim/FastQC（MultiQC 空报告已修复）、火山图修复、`results_dir` 输出根目录参数化、物种预设映射、版本记录。

仍遗留的主要事项：

1. KEGG 富集依赖外网，离线节点自动跳过（附录 A 提供离线 gmt 备选方案）。
2. 批次校正参数 `-b` 未配置化（阶段 3）。
3. 端到端运行验收与微型测试数据集（阶段 3 `tests/`）。
4. CI、slurm profile、CONTRIBUTING（阶段 3）。
