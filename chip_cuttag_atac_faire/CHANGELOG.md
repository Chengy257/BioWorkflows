...
- **WSL real-run validation round for the v0.5 diffbind stage (2026-09-10)**: `tests/run_test.sh --diffbind --real-run --reads 6000` runs green end to end against a live DiffBind/DESeq2 (2x2 contrast with a paired batch block): `DB_results.tsv` (24 summit-recentered consensus regions with Fold/p-value/FDR), `DB_significant.tsv`, MA/volcano/PCA plots, and sessionInfo all verified on real output. Three vignette-era API findings folded back: (1) the sample sheet must use DiffBind's own column names `Peaks`/`PeakCaller` (a `PeakFile` column is silently ignored, yielding an empty peak set) with `PeakCaller` = narrowpeak|broadpeak per file suffix — replacing the PeakFormat/ScoreCol/LowerBetter overrides; (2) `dba.count`'s first formal is `DBA` (`dba =` is rejected) and `dba.contrast` no longer accepts `reorderDBA`; (3) `dba.contrast`'s `block` takes a logical vector aligned with the sample sheet (a character vector of batch labels is the old API) — the batch block is now the first-level mask, the classic paired design. The harness assertion for the scenario's sample count was also scenario-adjusted (8 sorted BAMs under `--diffbind`, same as `--replicate`).
