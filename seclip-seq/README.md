...
| | `fastq_sort` | `fastq-sort --id` (ea-utils) + bgzip, deterministic order before alignment |
...
1. **Fresh server**: `mamba env create -f workflow/environment.yaml` (all-in-one environment `seclip-seq`, pinning snakemake-minimal 7.32.4 / STAR 2.7.10b / samtools 1.17 / cutadapt 4.6 / umi-tools 1.1.4 / ea-utils / fastqc 0.11.9 / multiqc 1.21 / pureclip 1.3.1);
...
4. **ea-utils fallback**: `rule fastq_sort` depends on `fastq-sort` (few conda builds); if a server cannot solve ea-utils, switch the rule to an equivalent name-based sort (see the note in `workflow/environment.yaml`).
