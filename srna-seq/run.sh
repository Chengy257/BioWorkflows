...
PROJECT_DIR=""
CONFIG_PATH=""
SOFTWARE_PATH="${SRNA_SOFTWARE_CONFIG:-}"
EXTRA_CONFIG=""
JOBS="${SRNA_JOBS:-10}"
PROFILE_REQUEST="${RUN_PROFILE:-auto}"
LOG_PATH="${SRNA_LOG:-snakemake.logs.txt}"
DRY_RUN=false
VALIDATE_ONLY=false
...
QUIET=false
QUEUE="${SRNA_QUEUE:-}"
PARTITION="${SRNA_PARTITION:-}"
MEMORY="${SRNA_MEMORY:-}"
RUNTIME_MIN="${SRNA_RUNTIME_MIN:-}"
SGE_MEMORY_RESOURCE="${SRNA_SGE_MEMORY_RESOURCE:-h_vmem}"
SCHEDULER_EXTRA="${SRNA_SCHEDULER_EXTRA:-}"
RETRIES="${SRNA_RETRIES:-0}"
LATENCY_WAIT="${SRNA_LATENCY_WAIT:-}"
MAX_JOBS_PER_SECOND="${SRNA_MAX_JOBS_PER_SECOND:-}"
MAX_STATUS_CHECKS_PER_SECOND="${SRNA_MAX_STATUS_CHECKS_PER_SECOND:-}"
...
Single pipeline:
  QC and SE trimming, sequential bowtie cascade filter over sncRNA
  classes, genome alignment, per-class count and RPM matrices
...
if [[ ${#POSITIONAL[@]} -ge 3 && "$JOBS" == "${SRNA_JOBS:-10}" ]]; then
    JOBS="${POSITIONAL[2]}"
fi
...
# the repository default (config/resources.yaml). Injected as SRNA_RESOURCES_CONFIG.
if [[ -n "${SRNA_RESOURCES_CONFIG:-}" ]]; then
    RESOURCES_PATH="$(resolve_path "$SRNA_RESOURCES_CONFIG")"
...
export SRNA_RESOURCES_CONFIG="$RESOURCES_PATH"
...
if [[ -n "$EXTRA_CONFIG" ]]; then
    export SRNA_EXTRA_CONFIG="$EXTRA_CONFIG"
fi
...
export SRNA_SOFTWARE_CONFIG="$SOFTWARE_PATH"
...
if [[ -n "$CONFIG_PATH" ]]; then
    export SRNA_CONFIG="$CONFIG_PATH"
else
    unset SRNA_CONFIG || true
fi
...
if [[ "$QUIET" != true ]]; then
    info "srna-seq launcher version: $SCRIPT_VERSION"
    info "Project: $PROJECT_DIR"
    info "Config: ${CONFIG_PATH:-$DEFAULT_CONFIG}"
    info "Software config: $SOFTWARE_PATH"
    info "Resources config: $RESOURCES_PATH"
    info "Software environment: ${SRNA_SOFTWARE_TYPE:-system}${SRNA_ENV_PREFIX:+ ($SRNA_ENV_PREFIX)}"
