...
#
# Usage:
#   bash tests/run_test.sh [--reads N] [--keep] [--real-run] [--deg] [--novel-mirna] [--help]
...
#     --novel-mirna  dry-run the optional novel-miRNA scenario: the
#                 novel_mirna stage is enabled with the generated
#                 ref/miRNA.fa as the mature reference; asserts that the
#                 DAG contains the novel_mirna rule
...
Usage:
  ...
  --deg       dry-run the optional differential-expression scenario (group
              ...
              that the DAG contains the deg_deseq2 rule
  --novel-mirna  dry-run the optional novel-miRNA scenario (novel_mirna
              enabled with ref/miRNA.fa as the mature reference); asserts
              that the DAG contains the novel_mirna rule
...
if [[ "$NOVEL" == 1 && "$REAL_RUN" == 1 ]]; then
    echo "[ERROR] --novel-mirna is a dry-run-only scenario (the novel stage needs the mirdeep-p2 conda env); drop --real-run" >&2
    exit 1
fi
MODE="dry-run"
[[ "$REAL_RUN" == 1 ]] && MODE="real-run"
[[ "$DEG" == 1 ]] && MODE="${MODE}+deg"
[[ "$NOVEL" == 1 ]] && MODE="${MODE}+novel-mirna"
...
if [[ "$NOVEL" == 1 ]]; then
    # Novel-miRNA scenario: the generated ref/miRNA.fa doubles as the known
    # mature-miRNA reference; the generated config already carries the genome
    # fasta (ref/genome.fa), so enabling the section adds one novel_mirna
    # job per sample to the DAG.
    cat >> "$WORK_DIR/config.yaml" <<'EOF'
novel_mirna:
  enabled: true
  mature_fasta: "ref/miRNA.fa"
EOF
    echo "[test]   novel-mirna scenario: stage enabled (mature reference = ref/miRNA.fa)"
fi
...
if [[ "$REAL_RUN" == 1 ]]; then
    ...
    if [[ "$NOVEL" != 1 ]]; then
        # Default-off contract: the novel-miRNA stage must be absent from the DAG.
        if grep -q "novel_mirna" "$CAPTURE_LOG" "$SNAKE_LOG" 2>/dev/null; then
            echo "  FAIL  default DAG must not contain rule novel_mirna"; FAIL=1
        else
            echo "  PASS  default DAG does not contain rule novel_mirna"
        fi
    fi
...
if [[ "$DEG" == 1 || "$NOVEL" == 1 ]]; then
    echo "[test] [INFO] optional-stage scenario: docs/dag_test.svg regeneration skipped (keeps the default-scenario DAG)"
