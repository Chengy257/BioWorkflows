...
- [x] Nucleotide stats: `bam2nuc` writes `{sample}.deduplicated.nucleotide_stats.txt`
  ...
  The `bam2nuc_sample` genome-folder path fix (totals sit one dirname up,
  not three) and the removal of the `--genomic_composition` option (Perl
  Getopt prefix-resolves it to `--genomic_composition_only`, so the BAM was
  never processed) are verified in the 2026-09-07 full real-run.
- [x] Per-sample report HTML: `bismark2report --output` takes the file name
  VERBATIM (passing `{sample}` writes a file literally named `{sample}`); the
  rule passes `{sample}.html`.
...
- [x] Per-sample `--parallel` alignment: Bismark 0.24 rejects `--basename`
...
- [x] Wire the lightweight CI job (plan Task D3, repository-root
  ...
  Added 2026-09-07 on `feature/seclip-srna-bs-v0.1` (seclip-seq / srna-seq /
  bs-seq jobs; chip and rna-seq jobs untouched); previously cancelled for
  v0.1 by user adjudication (2026-09-06, early scope closure). Locally,
  `make test` + `bash tests/run_test.sh --reads 2000` remain the reference
  bar.
...
adjudication (interrupt all execution; archive, do not fix), and synced on
the 2026-09-07 documentation pass (user re-opened the remaining v0.1 scope):

- [x] `README.md` (~L134-203, outside the naming table): the results
  ...
  of the naming table.
- [x] `docs/user-guide.md` (~§4/§7/§8/FAQ): the directory trees, results
...
- [x] `workflow/rules/align.smk` / `methylation.smk` / `common.smk` header
  comments: still narrate the pre-reconciliation chain and "Phase D must
  confirm" caveats in places.
- [x] `example/config.yaml` `merge_cpg` comment: references the old
  `{sample}.CpG_merged.tsv.gz` name.
- [x] `srna-seq/CHANGELOG.md`: record the D2-window behavior changes
  (multiqc `--filename` pin in `meta.smk`; R1/R2 trim artifact guard renames
  in `upstream.smk`) per its own CONTRIBUTING rules.
- [x] `seclip-seq/workflow/environment.yaml` comment: references
