...
if command -v snakemake >/dev/null 2>&1; then
    snakemake -s workflow/Snakefile --lint >/tmp/lint_bsseq.txt 2>&1 || true
    # Known baseline: the external-runtime conda advice (no rule ships a
    # conda: directive by design) and the common.smk helper-style warning.
    unexpected=$(grep -E '^    \* ' /tmp/lint_bsseq.txt \
        | grep -v 'Specify a conda environment or container for each rule.:' \
        | grep -v 'Mixed rules and functions in same snakefile.:' || true)
    if [[ -z "$unexpected" ]]; then
        note "  OK  snakemake --lint (external-runtime conda + helper-style warnings ignored)"
    else
        note "  FAIL snakemake --lint (see /tmp/lint_bsseq.txt)"; FAIL=1
    fi
else
