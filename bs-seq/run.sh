...
PROJECT_DIR=""
CONFIG_PATH=""
SOFTWARE_PATH="${BSSEQ_SOFTWARE_CONFIG:-}"
EXTRA_CONFIG=""
JOBS="${BSSEQ_JOBS:-10}"
PROFILE_REQUEST="${RUN_PROFILE:-auto}"
LOG_PATH="${BSSEQ_LOG:-snakemake.logs.txt}"
DRY_RUN=false
VALIDATE_ONLY=false
...
QUIET=false
QUEUE="${BSSEQ_QUEUE:-}"
PARTITION="${BSSEQ_PARTITION:-}"
MEMORY="${BSSEQ_MEMORY:-}"
RUNTIME_MIN="${BSSEQ_RUNTIME_MIN:-}"
SGE_MEMORY_RESOURCE="${BSSEQ_SGE_MEMORY_RESOURCE:-h_vmem}"
SCHEDULER_EXTRA="${BSSEQ_SCHEDULER_EXTRA:-}"
RETRIES="${BSSEQ_RETRIES:-0}"
LATENCY_WAIT="${BSSEQ_LATENCY_WAIT:-}"
MAX_JOBS_PER_SECOND="${BSSEQ_MAX_JOBS_PER_SECOND:-}"
MAX_STATUS_CHECKS_PER_SECOND="${BSSEQ_MAX_STATUS_CHECKS_PER_SECOND:-}"
...
Single pipeline:
  optional Trim Galore trimming, bisulfite genome preparation, Bismark
  alignment, deduplication, methylation extraction with per-sample
  cytosine reports, and run-level QC (Bismark summary + MultiQC)
...
if [[ ${#POSITIONAL[@]} -ge 3 && "$JOBS" == "${BSSEQ_JOBS:-10}" ]]; then
    JOBS="${POSITIONAL[2]}"
fi
...
# the repository default (config/resources.yaml). Injected as BSSEQ_RESOURCES_CONFIG.
if [[ -n "${BSSEQ_RESOURCES_CONFIG:-}" ]]; then
    RESOURCES_PATH="$(resolve_path "$BSSEQ_RESOURCES_CONFIG")"
...
export BSSEQ_RESOURCES_CONFIG="$RESOURCES_PATH"
...
if [[ -n "$EXTRA_CONFIG" ]]; then
    export BSSEQ_EXTRA_CONFIG="$EXTRA_CONFIG"
fi
...
export BSSEQ_SOFTWARE_CONFIG="$SOFTWARE_PATH"
...
if [[ -n "$CONFIG_PATH" ]]; then
    export BSSEQ_CONFIG="$CONFIG_PATH"
else
    unset BSSEQ_CONFIG || true
fi
...
if [[ "$QUIET" != true ]]; then
    info "bs-seq launcher version: $SCRIPT_VERSION"
    info "Project: $PROJECT_DIR"
    info "Config: ${CONFIG_PATH:-$DEFAULT_CONFIG}"
    info "Software config: $SOFTWARE_PATH"
    info "Resources config: $RESOURCES_PATH"
    info "Software environment: ${BSSEQ_SOFTWARE_TYPE:-system}${BSSEQ_ENV_PREFIX:+ ($BSSEQ_ENV_PREFIX)}"
