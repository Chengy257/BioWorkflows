...
group_contrast_plot <- function(res,data,name){
    ...
    v_p <- ggplot(data,aes(x=log2FoldChange,y =-log10(padj),color=type))+
        ...
        geom_text(data = anno,aes(x=-8,y=100,label=paste0("Down: ",anno$Freq[1]),family=c("mono"),frontface=c("plain")),color="black")+
        geom_text(data = anno,aes(x=8,y=100,label=paste0("Up: ",anno$Freq[3]),family=c("mono"),frontface=c("plain")),color="black")
