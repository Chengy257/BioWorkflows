...
### Added
- Configurable Trim Galore via a new `trim:` config section (`quality` / `stringency` / `error_rate` / `extra`, mirroring the chip workflow) wired into both `trimAdapter_SE` and `trimAdapter_PE`, plus optional UMI 5' clipping via `umi:` (`enabled` / `read1_len` / `read2_len`), which appends per-sample `--clip5pNbases` to the STAR command (PE renders two comma-separated values as required by STAR 2.7.10b; verified against libraries with an 8bp random UMI + fixed T anchor at position 9). Both sections are optional and default to reproducing the previous hardcoded commands byte-identically; enabling `umi:` clipping while `star_extra_args` also contains `--clip5pNbases` fails config validation. `umi_tools extract`/`dedup` integration was evaluated and deferred (see `docs/TODO.md`).
- `tests/run_test.sh --dry-run-only`: a deterministic local gate that stops after the dry-run (skipping the end-to-end run and the output assertions that need real outputs), for machines without the full R runtime. Default behavior without the flag is unchanged.
...
### Changed
- Raw FASTQ discovery now also accepts the common sequencer-delivery names `{id}_R1/_R2.fastq.gz` and `{id}_R1/_R2.fq.gz`; the original `{id}_1/_2` patterns keep priority, so existing projects (and the regression-test baseline DAG) resolve identically, and the manual `_1/_2` symlink workaround for deliveries is no longer needed.
